    ...
    <form action="index.php?uc=validerFrais&action=valideFrais" method="post">
        ...
        <table>
            <?php 
                if (empty($lesFraisHorsForfait)) {
                    ?><div>Il n'y a pas de frais hors forfait.</div><?php
                } else {
            ?>
                    ...
                    <tr>
                        ...
                        ?>      
                                <td><a href="index.php?uc=validerFrais&action=supprimerFrais&idHorsForfait=<?php echo $idHorsForfait ?>" 
                                        onclick="return confirm('Voulez-vous vraiment supprimer ce frais?');" class='btn'>Supprimer</a></td>
        ...
        ?>
        <div>
            <input class="zone"type="reset" /><input class="zone" <?php if ($dejaValide){echo $disable;}?> type="submit" value="VALIDER LA FICHE"/>
        </div>
    </form>
</div>
